...
if(isset($_GET['act'])){
	if($_GET['act'] == 'info'){
		...
		if(is_writable($lokasiberkas)){
			...
			$outs['error'] = 0;
			$outs['message'] = file_exists($lokasiberkas."/".$fnames) ? $weburl."/".$fnames : 'gagal';
		}
		header("Content-type: application/json; charset=utf-8");
		echo json_encode($outs);
		die();
